# install.packages("ggplot2")   # 安装 ggplot2 包（已注释）
...
# ========== 定义输入输出文件 ==========
inputFile = "input.txt"     # 输入文件：表达矩阵（行=样本，列=Type+基因）
outFile = "boxplot.pdf"     # 输出文件：分面箱线图 PDF

# setwd("D:\\biowolf\\bioR\\10.boxplotFacet")   # 设置工作目录（已注释）
...
for (gene in colnames(rt)[2:ncol(rt)]) {                # 遍历除 Type 外的每个基因列
  ...
  if (length(levels(factor(rt1$Type))) > 2) {         # 如果分组数 > 2
    test = kruskal.test(expression ~ Type, data = rt1)   # 用 Kruskal-Wallis 检验
    p = test$p.value
  } else {                                            # 如果分组数 = 2
    test = wilcox.test(expression ~ Type, data = rt1)    # 用 Wilcoxon 秩和检验
    p = test$p.value
  }
...
print(p1)   # 在 RStudio 面板中显示

# ========== 输出图片 ==========
pdf(file = outFile, width = 9, height = 5)   # 打开 PDF 设备
print(p1)                                     # 输出到 PDF
dev.off()                                     # 关闭设备，保存文件